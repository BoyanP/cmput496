...
window.onload = function(){
  ...
  setInterval(function(){
    ...
    ctx.fillStyle="#00000";
		...
		//console.log(canvas.width);
		ctx.clearRect(canvas.height-paddle.height,0,canvas.width,paddle.height);
    ctx.beginPath();
    ...
    ctx.stroke();
		...
		ctx.fillRect(paddle.top, paddle.left, paddle.width, paddle.height);
...
function movePaddle(event){
  ...
  var canvasWIDTHOFFSET  = canvas.offsetWidth;
  var xOffset = canvas.offsetLeft;
  var realOFFSET = canvasWIDTH / canvasWIDTHOFFSET;
  //console.log(xOffset);
  //var yOffset = canvas.offsetTop;
  context = canvas.getContext("2d");
  context.clearRect(last_rectX, paddle.left, paddle.width+1,paddle.height);
  
  context.fillStyle = "#FF0000";
  var mouseX = event.x - xOffset;
  console.log("event.pageX: " +event.x );
  console.log("mouseX: " +mouseX);
  console.log("paddleWidth: " + paddleWidth)

  function paddleX( mousex){
    console.log("inside paddleX: " + mouseX);
    var canvas = document.getElementById('gameCanvas');
    var max_X = canvas.offsetWidth;
    ...
    if(mousex <= (paddleWidth / 2) ){
      ...
      x = (realOFFSET * mousex) - (paddleWidth/1.7);
      console.log(mousex);
